function intersectPoints = segSplineIntersection(basicFunctions, segsParam, opt)

n = size(basicFunctions,1);
len = size(segsParam,1);
...
for i = 1:len
    xorg = segsParam(i,1);
    yorg = segsParam(i,2);
    xvec = segsParam(i,3)-segsParam(i,1);
    yvec = segsParam(i,4)-segsParam(i,2);
        ...
        for j = 1:n
            ...
            if (~isempty(xarray))&&(~isempty(fxarray))
                for kl = 1:rootnum
                    ...
                    Vec1 = [(xcoord-segsParam(i,1)) (ycoord-segsParam(i,2))];
                    Vec2 = [(xcoord-segsParam(i,3)) (ycoord-segsParam(i,4))];
                    tmpProduct = dot(Vec1,Vec2);
                    if tmpProduct <= 0
