function intersectPoints = lineSplineIntersection(basicFunctions, linesParam, opt)

n = size(basicFunctions,1);
len = size(linesParam,1);
...
for i = 1:len
    ...
    if ~isnan(k)
        ...
        for j = 1:n
            ...
            fun = @(t)bonex+btwox*t+bthrx*(t^2)+bfoux*(t^3)-b;
            [xarray,fxarray,rootnum] = multiRoot(fun,0,1);
            if (isempty(xarray))||(isempty(fxarray))
                [xarray,fxarray,rootnum] = multiRoot(fun,0,1, 1000);
            end
